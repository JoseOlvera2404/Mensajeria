...
export const sendFriendRequest = async (req, res) => {
    try {
        ...
        const existing = await pool.query(`SELECT id, status
       FROM mensajeria.friendships
       WHERE 
       (requester_id=$1 AND addressee_id=$2)
       OR
       (requester_id=$2 AND addressee_id=$1)`, [requesterId, userId]);
        ...
        const result = await pool.query(`INSERT INTO mensajeria.friendships
       (requester_id, addressee_id)
       VALUES ($1,$2)
       RETURNING id,status`, [requesterId, userId]);
        return res.json({
...
export const acceptFriendRequest = async (req, res) => {
    try {
        ...
        const result = await pool.query(`UPDATE mensajeria.friendships
       SET status='accepted'
       WHERE id=$1
       AND addressee_id=$2
       AND status='pending'
...
export const blockUser = async (req, res) => {
    try {
        ...
        await pool.query(`DELETE FROM mensajeria.friendships
       WHERE
       (requester_id=$1 AND addressee_id=$2)
       OR
       (requester_id=$2 AND addressee_id=$1)`, [userId, userIdToBlock]);
        // crear relación bloqueada
        await pool.query(`INSERT INTO mensajeria.friendships
       (requester_id, addressee_id, status)
       VALUES ($1,$2,'blocked')`, [userId, userIdToBlock]);
        return res.json({
...
export const deleteFriend = async (req, res) => {
    try {
        const userId = req.userId;
        const { id } = req.params;
        const result = await pool.query(`DELETE FROM mensajeria.friendships
       WHERE
       (requester_id=$1 AND addressee_id=$2)
       OR
...
export const unblockUser = async (req, res) => {
    try {
       ...
       AND addressee_id=$2
       AND status='blocked'
       RETURNING id`, [userId, userIdToUnblock]);
        if (result.rows.length === 0) {
            return res.status(404).json({
