...
export const sendMessage = async (req, res) => {
    try {
        ...
        const member = await pool.query(`SELECT 1
       FROM mensajeria.conversation_members
       WHERE conversation_id=$1
       AND user_id=$2`, [conversationId, senderId]);
        ...
        const result = await pool.query(`SELECT mensajeria.send_message(
        $1,$2,$3,'text'
      ) AS id`, [conversationId, senderId, encryptedContent]);
        ...
        const io = getIO();
        const members = await pool.query(`SELECT user_id
       FROM mensajeria.conversation_members
       WHERE conversation_id=$1`, [conversationId]);
        ...
        if (botId) {
            const isBotConversation = members.rows.some((m) => m.user_id === botId);
            if (isBotConversation && senderId !== botId) {
                const aiReply = await askAI(conversationId, content);
                const encryptedReply = encrypt(aiReply);
                const botMessage = await pool.query(`SELECT mensajeria.send_message(
            $1,$2,$3,'text'
          ) AS id`, [
                    conversationId,
                    botId,
...
export const markMessagesAsRead = async (req, res) => {
    try {
        ...
        await pool.query(`INSERT INTO mensajeria.message_reads
       (message_id,user_id)
       VALUES ($1,$2)
       ON CONFLICT DO NOTHING`, [messageId, userId]);
        if (conversationId) {
            await pool.query(`UPDATE mensajeria.conversation_members
         SET last_read_message_id=$1,
             unread_count = 0
         WHERE conversation_id=$2
         AND user_id=$3`, [messageId, conversationId, userId]);
            const io = getIO();
...
export const sendFileMessage = async (req, res) => {
    try {
        ...
        const message = await pool.query(`SELECT mensajeria.send_message(
        $1,$2,NULL,$3
      ) AS id`, [conversationId, senderId, messageType]);
        const messageId = message.rows[0].id;
        await pool.query(`INSERT INTO mensajeria.message_attachments
