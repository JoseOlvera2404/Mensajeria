...
export const createPrivateConversation = async (req, res) => {
    try {
        ...
        const [user1, user2] = userId < otherUserId
            ? [userId, otherUserId]
            : [otherUserId, userId];
        const existing = await pool.query(`SELECT conversation_id
       ...
       WHERE user1=$1 AND user2=$2`, [user1, user2]);
        ...
        const conv = await pool.query(`INSERT INTO mensajeria.conversations
       (type, created_by)
       VALUES ('private',$1)
       RETURNING id`, [userId]);
        const conversationId = conv.rows[0].id;
        await pool.query(`INSERT INTO mensajeria.private_conversations
       (conversation_id,user1,user2)
       VALUES ($1,$2,$3)`, [conversationId, user1, user2]);
        await pool.query(`INSERT INTO mensajeria.conversation_members
       (conversation_id,user_id)
       VALUES
       ($1,$2),
       ($1,$3)`, [conversationId, userId, otherUserId]);
        return res.json({
    ...
    try {
       ...
       VALUES ($1,$2,'admin')`, [conversationId, userId]);
        for (const memberId of members) {
            await pool.query(`INSERT INTO mensajeria.conversation_members
         (conversation_id,user_id)
         VALUES ($1,$2)`, [conversationId, memberId]);
        }
        return res.json({
